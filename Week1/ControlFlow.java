  ...
  int[] intArr = {4, 3, 5};
    ...
    */
     public static int[] orderArr(int[] intArr){
        ...
        int[] arrC = new int[intArr.length];

        //==================Even Case
        if(intArr.length == 1){
            return intArr;
        }
        if(intArr.length>1&&intArr.length%2 == 0){
            
            int len = intArr.length/2;
            int[] arrA = new int[intArr.length/2];
            int[] arrB = new int[intArr.length/2];
            

            for(int i = 0; i< len ; i++){
               arrA[i] = intArr[i]; 
            }
            for(int i = len; i < intArr.length ; i++){
               arrB[i-len] = intArr[i]; 
            }

            int[] orderA = orderArr(arrA);
            int[] orderB = orderArr(arrB);
        ...
        if(intArr.length>1&&intArr.length%2 != 0){
            ...
            int[] arrA = new int[(intArr.length-1)/2];
            
            int[] arrB = new int[(intArr.length+1)/2];

            for(int i = 0; i<((intArr.length-1)/2) ; i++){
               arrA[i] = intArr[i];
            }

            for(int i = ((intArr.length-1)/2); i < intArr.length ; i++){
               arrB[i-(intArr.length/2)] = intArr[i]; 
            }
