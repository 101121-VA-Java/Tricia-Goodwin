...
function populateData(response) {
    ...
    let dataSection = document.getElementsById('data');

    let pokeName = response.name;
    let pokeWeight = response.weight;
    let pokeMove = response.moves[1].move.name;
    let pokeMove1 = response.moves[0].move.name;
    ...
    nameTag.innerHTML = ` Pokemon name: ${pokeName}`;
